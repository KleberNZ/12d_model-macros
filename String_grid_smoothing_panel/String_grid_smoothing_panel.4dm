...
void mainPanel(){
    ...
    Named_Tick_Box ntb_use_median_filter = Create_named_tick_box("Use median spike filter",1,"use_median_filter");
    ...
    // Signature: Named_Tick_Box Create_named_tick_box(Text title,Integer state,Text response)
    Named_Tick_Box ntb_use_gaussian_filter = Create_named_tick_box("Use Gaussian smoothing",1,"use_gaussian_filter");
    ...
    // Signature: Real_Box Create_real_box(Text title,Message_Box message)
    Real_Box rb_max_vertical_change = Create_real_box("Maximum vertical change",cmbMsg);

    Set_data(ib_median_window_size,3);
    Set_data(rb_spike_tolerance,0.100);
    Set_data(ib_min_valid_neighbours,3);
    ...
    Set_border(vg_median_filter,"Median Spike Filter");
    Set_border(vg_gaussian_filter,"Gaussian Smoothing");
    ...
    Set_tooltip(rb_spike_tolerance,
        "Maximum allowed difference from neighbourhood median. Smaller values alter more cells.");
    ...
    while(doit)
    {
        ...
        case Get_id(process) :
        {
            if(cmd == "process")
            {
                ...
                if(!use_median_filter && !use_gaussian_filter) {
                    valid = 0;
