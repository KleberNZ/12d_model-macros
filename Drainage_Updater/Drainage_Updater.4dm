...
**   Type:                 SOURCE
**
**   Brief description: Update DNE from as-built (LIDs, SUMPs, Pipe ILs)
**
**---------------------------------------------------------------------
**   Minimal-safe tweaks:
**   • Source_Box handled per manual: TRUE / NO_NAME; empty OK; own counts.
**   • No re-declared loop vars; strict bounds checks.
**   • LID points-only; Pipes 2-pt lines; strict 3D reads.
**   • Cover RL = Manual via node attribute ("cover rl mode"=2) + commit.
**---------------------------------------------------------------------*/
#define DEBUG_FILE      0
                          ...
                          Real px, Real py, Real tol, Real &best_z)
{
  ...
  return (found && best_d2<=tol2) ? 1 : 0;
}

// Generic nearest vertex (3D only) — used for SUMP
...
// Drainage classification + counts
Integer classify_drainage(Element e, Integer &dir, Integer &nsegs, Integer &npits)
{
  ...
  if(nsegs > (npits-1)) nsegs = npits-1;  // extra guard
...
// Source_Box per manual: TRUE / NO_NAME; empty OK.
// Returns: 1 = present (elements may be 0), 0 = omitted, -1 = error (−2 or FALSE)
Integer resolve_source(Source_Box bx, Dynamic_Element &de, Integer &count)
{
  count=0;
  Integer rc = Validate(bx, de);
  if(rc == TRUE){ Get_number_of_items(de, count); return 1; }
  if(rc == NO_NAME) return 0;
  return -1;
}
...
void mainPanel(){
  ...
  // Drainage model (must exist)
  Model_Box  dmodel_box = Create_model_box("Drainage model", cmbMsg, CHECK_MODEL_MUST_EXIST);
    ...
    switch(id)
    {
      ...
      case Get_id(process):
      {
        if(cmd == "process")
        {
          // Drainage model must exist
          Model dmodel;
          Integer rcM = Validate(dmodel_box, CHECK_MODEL_MUST_EXIST, dmodel);
          if(rcM != MODEL_EXISTS){ Set_error_message(dmodel_box, "Pick a valid drainage model."); break; }

          Dynamic_Element d_elts; Integer d_count=0;
          if(Get_elements(dmodel, d_elts, d_count)!=0 || d_count==0){ Set_data(cmbMsg,"Drainage model is empty."); break; }

          // Resolve as-built sources per manual
          Dynamic_Element lid_elts, sump_elts, pipe_elts;
          Integer lid_cnt=0, sump_cnt=0, pipe_cnt=0;

          Integer lid_rc  = resolve_source(lid_src,  lid_elts,  lid_cnt);
          if(lid_rc  < 0){ Set_error_message(lid_src,  "LID source: invalid selection.");  break; }

          Integer sump_rc = resolve_source(sump_src, sump_elts, sump_cnt);
          if(sump_rc < 0){ Set_error_message(sump_src, "SUMP source: invalid selection."); break; }

          Integer pipe_rc = resolve_source(pipe_src, pipe_elts, pipe_cnt);
          if(pipe_rc < 0){ Set_error_message(pipe_src, "PIPES source: invalid selection."); break; }

          // Use flags: present & non-empty only
          Integer use_lid  = (lid_rc  == 1 && lid_cnt  > 0);
          Integer use_sump = (sump_rc == 1 && sump_cnt > 0);
          Integer use_pipe = (pipe_rc == 1 && pipe_cnt > 0);

          if(!use_lid && !use_sump && !use_pipe){ Set_data(cmbMsg,"Nothing to do (no as-built sources)."); break; }
          ...
          // ---------------- PITS (LID + SUMP) ----------------
          if(use_lid || use_sump){
            Print("== PITS ==  LID="+(use_lid?"yes":"no")+", SUMP="+(use_sump?"yes":"no")+"\n");
              ...
              for(p=1;p<=npits;p++){
                ...
                if(use_lid){
                  ...
                  if(nearest_lid_point(lid_elts, lid_cnt, px,py, tol, lid_z)!=0){
                    ...
                    } else { lids_no++; Print("  pit "+To_text(p)+": LID set failed\n"); }
                ...
                if(use_sump){
                  ...
                  if(nearest_vertex_xy(sump_elts, sump_cnt, px,py, tol, sump_z)!=0){
                    if(Set_drainage_pit_float_sump(s,p,0)==0 &&
                       Set_drainage_pit_sump_level(s,p,sump_z)==0){
                      sumps_upd++;
          ...
          // ---------------- PIPES (2-pt lines) ----------------
          Print("== PIPES ==  "+(use_pipe?"running":"skipped")+"\n");
          if(use_pipe){
            ...
            for(i=1;i<=d_count;i++){
              ...
              for(sidx=1; sidx<=nsegs; sidx++){
                ...
                Real ax,ay,az,bx,by,bz; if(get_line_endpoints(best_e,ax,ay,az,bx,by,bz)!=0){
                  nomatch++; Print("  seg "+To_text(sidx)+": matched element not 2-pt\n"); continue;
                }
                ...
                if(Set_drainage_pipe_inverts(e, sidx, lhs, rhs)==0){
                  ...
                  Print("  seg "+To_text(sidx)+": IL set failed\n");
                }
              }
            }
